...
через абсолют поробовать-----2) Лого в шапке лучше через абсолют задать или  оставить так но сделать разные паддинги снизу и сверху? pad: 28px 0
посмотреть варианты в интернете------3) Как сделать стилизованную компоненту GridWrapper по аналогии Flexwrapper?
4) Нормально ли заголовок каждой секции делать через компоненту SectionTitle?  Ограничить по ширине и поискать что то типо такого &#10;
...
16) как лучше подменять картинки при адаптиве в секции PopularArtist через srsset или делать через медия запрос и скрывание элемента object-fit cover object-position start/left + liner gradient для div В котором картинка
17) Пятна как сделать на заднем фоне? Через before или after задать размеры и блюр
*/
