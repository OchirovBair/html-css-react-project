...
----------- 12) в файле PopularArtist задавал последнему div grid-area, но это значение прокидывается дальше внуть Journal и этот селектор применяется к последнему div перезатирая grid-area. Как это лучше пофиксить? черзе &>div:nth-child()
...
/*
-----------1) у кнопоки в секции Examples ее ширина подстраивается под ширину родителя. Как сделать так чтобы ширина родителя подстраиваевалась под контент? если убирать grid-area у div с карточками то вроде выстраиваеться колонка под кнопку? это нормально поведение и  justify-self: end; align-self: center; нормально прописывать
-----------2) header зацепил, хотя этого явно нет на макете. Или дучше сделать как обычную секцию? это нормально
-----------3) адаптив для картинок в секции Create и Subscribe. Правильно ли сделанно? попробоавть для контейнера задать width: 60vw + aspect-ratio
-----------4) кнопки на макете с акцентным цветом выше чем темные. Это лучше реализовать через проопсы? сделать что то среднее, возможно на макете рука дрогнула
*/

/*
-----------1) когда открываю меню бургер то позади сайт все равно прокручивается. Что делать или так и оставить?
2) нормально ли так добираться до элементтов см Subcribe > SubscribeSectionWrapper?
3) нормально ли сразу перебрасывать в колонну карточки в секциях PopularArtist ExampleSection?
4) нормально ли задавть какие либо размеры по типу height или width через clamp(${250}px, calc((100vw - 375px) / (1320 - 375) * (${458} - ${250}) + ${250}px), ${458}px);?
* */